#' The boot2pval function. Creates a bootstrap interval for a two sample test. Additionally outputs a graph showing the p-value.
#'
#' @param x1 the first vector sample.
#' @param x2 the second vector sample.
#' @param conf.level the confdience level for the confidence interval.
#' @param iter the number of iterations done in the bootstrap testing.
#' @param mudiff the tested/expected difference in means between the two samples.
#' @param test the type of test e.g two tailed or one tailed.
#'
#'
#' @return An output giving the pvalue as well as the graph displaying the t distribution.
#' @export
#'
#' @examples
